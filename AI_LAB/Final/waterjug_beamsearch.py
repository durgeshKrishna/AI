...
def heuristic(state, target):
    ...
    if x == target or y == target:
        return 0
    # Otherwise, heuristic is the minimum number of steps needed
...
def beam_search(start, target, beam_width=2):
    """
    Perform Beam Search to solve the Water Jug Problem.
...
def main():
    ...
    solution_path = beam_search(initial_state, TARGET, beam_width)
